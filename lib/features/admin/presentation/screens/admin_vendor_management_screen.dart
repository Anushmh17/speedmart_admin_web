...
class AdminVendorManagementScreen extends ConsumerStatefulWidget {
  const AdminVendorManagementScreen({
    ...
    this.title = 'Shop Owner Management',
    this.subtitle = 'Approve, assign & manage shop owners',
  });
...
class _AdminVendorManagementScreenState
  ...
  Widget build(BuildContext context) {
    ...
    final filteredVendors = vendors.where((v) {
      final isApprovalScreen = widget.initialFilter == 'pending';
      final isShopOwnersScreen = widget.initialFilter == 'active';

      if (isApprovalScreen) {
        return v.vendorStatus == VendorStatus.pendingApproval;
      }

      if (isShopOwnersScreen) {
        return v.vendorStatus == VendorStatus.approved && v.isShopLocationAssigned == true;
      }

      if (v.vendorStatus == VendorStatus.pendingApproval) {
        return false;
      }
